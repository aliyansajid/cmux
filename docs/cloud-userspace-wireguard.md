...
The Ports row leads with the VM service port. `vm.port_open` reports that port as
`remote_port` and the existing loopback endpoint as nullable `local_url` and
`local_port`: VM port `8000` may use local `127.0.0.1:64321`. The private URL remains
available as `url` and `private_url`; Copy VM Address copies it without creating a
forward. A missing private address shows connection guidance.
