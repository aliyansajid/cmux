...
| `vm open <id> <port> [--print]` | Open the canonical machine-port browser resource. HTTP uses an authenticated loopback forward; HTTPS keeps its private host and certificate identity. `url` and `private_url` identify the private URL. `remote_port` is the VM service port; nullable `local_url` and `local_port` report an existing local listener, whose ephemeral port can differ from the VM port. Copying and inspecting metadata do not create a forward. `--print` remains the explicit control-plane URL request. |
