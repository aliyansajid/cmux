 ...
 * keeps its state under /root but exposes its binary on a shared readable path.
 *
...
export function cmuxTuiInstallCommand(source: CmuxTuiSource): string {
  ...
  return [
    ...
    `if [ "$CMUX_TUI_USER" = root ]; then ${cmuxTuiLegacyInstallCommand(source)}; else ` +
      [
    ...
    // A migrated legacy binary may use its older baked pin. Leave its hooks to
    // ensureAgentHooks, which resolves that binary's own manifest; never pair
    // an old daemon with the live manifest's helper or unsupported hook verbs.
    `if ${pinnedFile(source.sha256, bin)}; then ` + [
    ...
    ].join(" && ") + `; else ${bin} --version; fi`,
  ].join(" && ");
}

/** Preserve a legacy image's baked pin while moving its binary out of /root. */
function cmuxTuiLegacyInstallCommand(source: CmuxTuiSource): string {
  const shared = shellQuote(CMUX_TUI_SHARED_BINARY_PATH);
  const bin = '"$CMUX_TUI_BIN"';
  const tmp = shellQuote(`${CMUX_TUI_SHARED_BINARY_PATH}.tmp`);
  const matches = (path: string) => `printf '%s  %s\n' "$CMUX_TUI_EXPECTED_SHA" ${path} | sha256sum -c >/dev/null 2>&1`;
  return [
    `mkdir -p /usr/local/lib/cmux "$(dirname "$CMUX_TUI_BIN")"`,
    `chmod 755 /usr/local/lib/cmux`,
    ...
    `if [ -x ${shared} ] && ${matches(shared)}; then :; else ${fetchTo(tmp, source.url)} && ${pinnedFile(source.sha256, tmp)} && chmod 755 ${tmp} && mv -f ${tmp} ${shared}; fi`,
    `chmod 755 ${shared}`,
    `ln -sfn ${shared} ${bin}`,
...
/** Called after layout selection; only legacy root machines need migration. */
...
// current_exe resolves the legacy symlink; the hook helper must be beside that
// real executable, including during hooks-only repair on an already moved VM.
function cmuxTuiHookPathSelector(): string {
  return `CMUX_TUI_HOOK_BIN="$(dirname "$(readlink -f "$CMUX_TUI_BIN")")/cmux-tui-hook"`;
}
...
export function cmuxTuiHooksReadyCommand(): string {
  return `${cmuxTuiLayoutSelector()} && ${cmuxTuiHookPathSelector()} && ${cmuxTuiHooksReadyCheck()}`;
}
