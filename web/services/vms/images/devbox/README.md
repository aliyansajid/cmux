...
Legacy images whose daemon still runs as root migrate their pinned binary from
`/root/.cmux/bin/cmux-tui` to `/usr/local/lib/cmux/cmux-tui` during attach-time
repair. Both the old path and `/usr/local/bin/cmux-tui` point to the shared file,
so a desktop or SSH work user can execute it. The migration preserves the baked
pin and inode; the hook helper is installed beside the real executable.
