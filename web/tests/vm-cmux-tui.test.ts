...
import { chmodSync, existsSync, mkdirSync, mkdtempSync, readFileSync, readlinkSync, statSync, rmSync, writeFileSync } from "node:fs";
...
describe("cmux-tui install and daemon commands", () => {
  test("legacy install migrates the baked binary to a readable path and keeps pin enforcement", () => {
    ...
    expect(command).toContain('if [ "$CMUX_TUI_USER" = root ]; then mkdir -p /usr/local/lib/cmux');
    expect(command).toContain("cut -d' ' -f1 /etc/cmux/cmux-tui-pin");
    expect(command).toContain(`then mv -f "$CMUX_TUI_BIN" '/usr/local/lib/cmux/cmux-tui'; fi`);
    ...
    expect(command).toContain("chmod 755 '/usr/local/lib/cmux/cmux-tui'");
    ...
    expect(cmuxTuiPinCheckCommand(source)).toContain(cmuxTuiLayoutCheckCommand());
    expect(cmuxTuiLayoutCheckCommand()).toContain('if [ "$CMUX_TUI_USER" = root ]');
  });

  test("legacy migration keeps the baked inode and version without fetching the live pin", () => {
    const dir = mkdtempSync(join(tmpdir(), "cmux-legacy-install-"));
    try {
      const rootBin = join(dir, "root/.cmux/bin/cmux-tui");
      const shared = join(dir, "usr/local/lib/cmux/cmux-tui");
      const body = "#!/bin/sh\nexit 0\n";
      mkdirSync(join(dir, "root/.cmux/bin"), { recursive: true });
      mkdirSync(join(dir, "usr/local/bin"), { recursive: true });
      mkdirSync(join(dir, "etc/cmux"), { recursive: true });
      writeFileSync(rootBin, body, { mode: 0o755 });
      const inode = statSync(rootBin).ino;
      const bakedSha = createHash("sha256").update(body).digest("hex");
      writeFileSync(join(dir, "etc/cmux/cmux-tui-pin"), `${bakedSha} old-commit\n`);
      const source = { url: "https://invalid.invalid/never-download", sha256: SHA, commit: COMMIT, builtAt: null, hookUrl: HOOK_URL, hookSha256: HOOK_SHA };
      const command = cmuxTuiInstallCommand(source)
        .replace(cmuxTuiLayoutSelector(), `CMUX_TUI_USER=root; CMUX_TUI_BIN=${rootBin}`)
        .replaceAll("/usr/local/", `${dir}/usr/local/`)
        .replaceAll("/etc/cmux/", `${dir}/etc/cmux/`)
        .replaceAll("sha256sum -c", process.platform === "darwin" ? "shasum -a 256 -c" : "sha256sum -c");
      for (let attempt = 0; attempt < 2; attempt++) {
        const result = spawnSync("sh", ["-c", command], { encoding: "utf8", timeout: 1000 });
        expect(result.status).toBe(0);
        expect(readFileSync(shared, "utf8")).toBe(body);
        expect(statSync(shared).ino).toBe(inode);
        expect(statSync(shared).mode & 0o777).toBe(0o755);
        expect(readlinkSync(rootBin)).toBe(shared);
        expect(readlinkSync(join(dir, "usr/local/bin/cmux-tui"))).toBe(shared);
      }
    } finally {
