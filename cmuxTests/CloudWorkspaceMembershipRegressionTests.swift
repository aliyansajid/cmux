...
@Suite("Cloud workspace membership lifecycle")
struct CloudWorkspaceMembershipRegressionTests {
    private let machine = SurfaceMachineID.cloud("membership-vm")
    ...
    func closedWorkspaceDoesNotReappear(useDelta: Bool) throws {
        ...
        #expect(terminalCounts(catalog.snapshot) == ["ws_quit": 2])
    ...
    func emptyWorkspaceCreationFollowsTheGraph() throws {
        ...
        #expect(Set(catalog.machines[machine]?.remoteWorkspaces?.map(\.id) ?? []) == ["ws_quit", "ws_empty"])
        let closed = try closingWorkspace(in: initial)
        ...
        // enters the catalog when the daemon publishes it. Empty rows remain
        // hidden in the sidebar, but lookup must permit intentional recreation.
        ...
        #expect(terminalCounts(catalog.snapshot).isEmpty)
        #expect(catalog.machines[machine]?.remoteWorkspaces?.map(\.id) == ["ws_empty", "ws_new"])
    ...
    private func expectClosedWorkspace(in catalog: SurfaceCatalog) throws {
        ...
        #expect(terminalCounts(snapshot).isEmpty)
        #expect(catalog.machines[machine]?.remoteWorkspaces?.map(\.id) == ["ws_empty"])
