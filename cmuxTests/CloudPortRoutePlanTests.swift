...
struct CloudPortRoutePlanTests {
    ...
    @Test("Proxy failures retain recovery controls until the browser leaves Cloud")
    func proxyFailureRetainsRecovery() {
        let state = CloudBrowserAccessState()
        state.showUnavailable("Private address unavailable")
        var opens = 0
        state.onOpenProxy = { opens += 1 }
        state.onOpenProxy?()
        #expect(opens == 1)
        state.showProxyFailure("Publication unavailable")
        #expect(state.failureMessage == "Publication unavailable")
        #expect(state.showsFailureAlert)
        #expect(state.onOpenProxy != nil)
        state.leave()
        #expect(state.onOpenProxy == nil)
