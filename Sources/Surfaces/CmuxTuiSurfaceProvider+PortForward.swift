...
extension CmuxTuiSurfaceProvider {
    ...
    func refreshCloudBrowserRoutes() {
        for resource in catalog.snapshot.resources(on: machine) where resource.kind != .terminal {
            for projection in catalog.projections(of: resource.id) {
                ...
                configureProxyFallback(browser, resource: resource, pane: (projection.workspaceID, projection.panelID))
            }
        }
    }
    ...
    func materializeBrowserPane(
        ...
        configureProxyFallback(browser, resource: resource, pane: pane)
        return pane
    }

    /// The native connection card owns retry; retain the publication escape
    /// hatch when a private port cannot connect, without exposing Desktop.
    private func configureProxyFallback(
        _ browser: BrowserPanel,
        resource: SurfaceResource,
        pane: (workspaceID: UUID, panelID: UUID)
    ) {
        guard resource.kind == .browser,
              let vmID = machine.cloudMachineID,
              let port = resource.id.forwardedPort ?? resource.port else { return }
        browser.cloudAccess.onOpenProxy = { [weak self, weak browser] in
            guard let self, let browser, self.isRegisteredInCatalog(),
                  self.browserPaneTasks[pane.panelID] == nil else { return }
            let generation = self.currentLifecycleGeneration
            let remoteURL = browser.cloudAccess.remoteURL
            self.browserPaneTasks[pane.panelID] = Task { @MainActor [weak self, weak browser] in
                guard let self, let browser else { return }
                defer { self.browserPaneTasks[pane.panelID] = nil }
                do {
                    let url = try await CloudPortProxy.url(vmID: vmID, port: port)
                    try Task.checkCancellation()
                    guard self.isCurrentLifecycleGeneration(generation), self.isRegisteredInCatalog(),
                          browser.cloudAccess.onOpenProxy != nil,
                          browser.cloudAccess.remoteURL == remoteURL else { return }
                    _ = try await CloudPortProxy.open(url, replacing: pane)
                } catch {
                    guard !Task.isCancelled, self.isCurrentLifecycleGeneration(generation),
                          browser.cloudAccess.onOpenProxy != nil,
                          browser.cloudAccess.remoteURL == remoteURL else { return }
                    browser.cloudAccess.showProxyFailure(CloudMachineLink.errorText(error))
                }
            }
        }
    }
