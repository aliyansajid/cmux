...
extension CmuxTuiSurfaceProvider {
    ...
    /// Describe the VM service and any existing local listener without changing routing.
    func portLink(port: Int) async throws -> CloudPortLink {
        let privateURL = try await portLinkURL(port: port)
        return CloudPortLink(
            remotePort: port,
            url: privateURL,
            privateURL: privateURL,
            localPort: await portForwards?.localPort(machineID: machineID, port: port)
        )
    }
