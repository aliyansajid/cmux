...
final class CloudTreeTerminalNavigationCoordinator {
    ...
    private weak var preferredTabManager: TabManager?
    ...
    func open(
        ...
        let windowScope = preferredTabManager.map { String(describing: ObjectIdentifier($0)) } ?? "global"
        let key = "cloud-terminal:\(windowScope):\(machine.rawValue):\(remoteWorkspaceID)"
    ...
    private func navigate(
        ...
        let workspaceIDs = preferredTabManager.map { Set($0.tabs.map(\.id)) }
        let projectionIndex = CloudTreeNodeBuilder.LocalProjectionIndex(snapshot: catalog.snapshot, workspaceIDs: workspaceIDs)
        let localWorkspaceID = projectionIndex.localWorkspaceShowing(
            remoteWorkspaceID: remoteWorkspaceID,
            placements: group.placements
        ) ?? openIn.flatMap { id in
            (workspaceIDs?.contains(id) ?? true) ? id : nil
        }
        ...
        let opened = try await catalog.projectGroupAsNewLocalWorkspace(
            ...
            focus: true,
            host: .appOptimistic(preferredTabManager: preferredTabManager),
            layout: layout
        )
