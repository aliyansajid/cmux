...
struct MachinesPanelView: View {
    ...
    init(chromeBackgroundColor: NSColor, defaultMachineStore: DefaultCloudMachineStore, tabManager: TabManager) {
        self.chromeBackgroundColor = chromeBackgroundColor
        _tabManager = ObservedObject(wrappedValue: tabManager)
        _viewModel = StateObject(wrappedValue: MachinesPanelViewModel(tabManager: tabManager, defaultMachineStore: defaultMachineStore))
    }
    ...
    static func bound(
        ...
        MachineRowActions(
            ...
            resizeDisk: { id, gib in
                ...
                if !launch(arguments: ["vm", "resize", id, "--disk", "\(gib)G"], preferredWindow: preferredWindow(), onDidMutate: onDidMutate) {
                    onDidMutate()
                }
            },
            resizeCPU: { id, cpu in
                onWillMutate(String(format: String(localized: "machines.operation.resize", defaultValue: "Resizing %@…"), id))
                if !launch(arguments: ["vm", "resize", id, "--cpu", "\(cpu)"], preferredWindow: preferredWindow(), onDidMutate: onDidMutate) { onDidMutate() }
            },
            resizeMemory: { id, gib in
                onWillMutate(String(format: String(localized: "machines.operation.resize", defaultValue: "Resizing %@…"), id))
                if !launch(arguments: ["vm", "resize", id, "--memory", "\(gib)G"], preferredWindow: preferredWindow(), onDidMutate: onDidMutate) { onDidMutate() }
            },
            promptUpgrade: {
